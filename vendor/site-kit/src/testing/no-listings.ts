...
const MONEY =
  /(?:aed|dhs?|د\.إ)\s*\d[\d,.]*\s*(?:m|k|mn|million|thousand)?|\b\d[\d,.]*\s*(?:m|k|mn|million)?\s*(?:aed|dhs?)\b/gi;

/** A price near any of these is describing one purchasable unit. */
const SPECIFIC_UNIT: readonly { id: string; re: RegExp }[] = [
...
const MARKET_LEVEL: readonly RegExp[] = [
...
function matchesAny(window: string, patterns: readonly RegExp[]): boolean {
  return patterns.some((re) => re.test(window));
}

export async function scanNoListings(distDir: string): Promise<ScanResult> {
  ...
  for (const file of files) {
    ...
    for (const match of text.matchAll(MONEY)) {
      ...
      const window = text.slice(
        ...
        Math.min(text.length, at + match[0].length + PROXIMITY),
      );

      // Market-level qualifier wins — this is data, not an offer.
      if (matchesAny(window, MARKET_LEVEL)) continue;
