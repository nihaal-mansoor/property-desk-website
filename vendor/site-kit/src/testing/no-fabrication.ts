...
/**
 * CLAUDE.md §1.2 — no fabricated credentials or trust signals.
 *
 * Invented brand names are fine. Invented authority is not. This catches the
 * patterns that creep in when writing marketing copy on autopilot.
 */
...
const RULES: readonly Rule[] = [
  ...
  },
  {
    ...
    pattern:
      /\b(?:founded|established)\s*(?:in\s*)?(?:19|20)\d{2}\b|\best\.\s*(?:19|20)\d{2}\b|\b(?:serving|trading|operating|in business|helping)(?:\s+\S+){0,3}\s+since\s+(?:19|20)\d{2}\b/gi,
  ...
  },
  {
    ...
    pattern:
      /\b(?:award[- ]winning|as featured (?:in|on)|as seen (?:in|on)|official partner|authoris?ed (?:agent|partner|dealer)|(?:we (?:are|'re)\s+)?(?:fully\s+|officially\s+)?(?:certified|accredited)\s+(?:agent|broker|brokerage|advis[eo]r|partner|consultant|firm|company|professional)s?)\b/gi,
...
export async function scanNoFabrication(distDir: string): Promise<ScanResult> {
